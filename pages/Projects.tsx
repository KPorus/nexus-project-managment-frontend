...
//         await api.updateProject(editingProject.id, { name, description, status });
//       } else {
//         // Create Mode (All Users)
//         // Use user._id instead of user.id
//         await api.createProject({ name, description }, user!._id);
//       }
//       handleCloseModal();
//       fetchProjects();
//     } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.length > 0 ? (
            filteredProjects.map((project) => (
              <div key={project.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col transition-shadow hover:shadow-md">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white truncate pr-2">{project.name}</h3>
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-4 flex-grow line-clamp-3 text-sm">{project.description}</p>
                ...
                {canEdit && (
                  <div className="flex space-x-2 mt-auto">
                    <Button variant="ghost" size="sm" onClick={() => handleOpenModal(project)}>Edit</Button>
            ...
            <div className="col-span-full flex flex-col items-center justify-center py-12 px-4 text-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
              <p className="text-gray-500 dark:text-gray-400 text-lg">No projects found matching the selected filter.</p>
              {filterStatus !== 'ALL' && (
                <button 
      ...
      >
        <form onSubmit={()=>{}} className="space-y-4">
          ...
          {editingProject && (
             ...
             >
               {Object.values(ProjectStatus).filter(s => s !== ProjectStatus.DELETED).map((s) => (
          ...
          <div className="flex justify-end pt-4">
            <Button type="button" variant="secondary" onClick={handleCloseModal} className="mr-3">Cancel</Button>
